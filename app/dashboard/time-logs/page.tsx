...
const taskData: Task[] = [
  {
    id: "PROJ-501",
    description: "Implement the Jira Timelog Table Component (FE)",
    logs: {
      "Mon 23": "3h 30m",
      "Tue 24": "4h 0m",
      "Wed 25": "1h 15m",
  ...
  {
    id: "PROJ-502",
    description: "Setup API endpoint for time log data fetching",
    logs: {
      "Mon 23": "2h 0m",
      "Tue 24": "1h 30m",
      "Wed 25": "2h 30m",
  ...
  {
    id: "SUPPORT-112",
    description: "Investigate production bug with user profile image scaling",
    logs: {
      "Mon 23": "—",
      "Tue 24": "—",
      "Wed 25": "—",
  ...
  {
    id: "PROJ-503",
    description: "Refactor auth service for better security",
    logs: {
      "Mon 23": "2h 30m",
      "Tue 24": "—",
      "Wed 25": "3h 0m",
  ...
  {
    id: "DOCS-10",
    description: "Update technical documentation for new API",
    logs: {
      "Mon 23": "—",
      "Tue 24": "2h 0m",
      "Wed 25": "1h 45m",
...
const dayHeaders: string[] =
  taskData.length > 0 ? Object.keys(taskData[0].logs) : [];
...
const TimeLogTable: React.FC = () => {
  ...
  const dailyTotals: Logs = dayHeaders.reduce(
    (acc, day) => ({ ...acc, [day]: 0 }),
    {},
  );
  ...
  const exportToCSV = (): void => {
    ...
    processedRows.forEach((task: ProcessedTask) => {
      const rowLogs: string = dayHeaders
        .map((day: string) => {
    ...
    // Add Grand Total row
    const dailyTotalValues: string = dayHeaders
      .map(
        (day: string) =>
    ...
    <div className="bg-[#F8F9FA] min-h-screen p-4 sm:p-8 font-sans">
      <div id="app" className="max-w-7xl mx-auto">
        <header className="mb-8">
          <h1 className="text-3xl sm:text-4xl font-extrabold text-[#172B4D] mb-2">
            Weekly Time Log
          </h1>
          <p className="text-lg text-[#42526E]">
            Review and export time spent across tasks for the week.
          </p>
        </header>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <thead className="bg-[#F4F5F7] sticky top-0 z-10">
                <tr>
                  <th
                    scope="col"
                    className="px-6 py-3 text-left text-xs font-medium text-[#42526E] uppercase tracking-wider min-w-[250px]"
                  >
                    Task Details
                  </th>
                  {dayHeaders.map((day: string) => (
                    <th
                      key={day}
                      scope="col"
                      className="px-6 py-3 text-right text-xs font-medium text-[#42526E] uppercase tracking-wider min-w-[80px]"
                    >
                      {day}
                    </th>
                  ))}
                  <th
                    scope="col"
                    className="px-6 py-3 text-right text-xs font-bold text-[#0052CC] uppercase tracking-wider bg-soft-gray/50 min-w-[100px]"
                  >
                    TOTAL
                  </th>
                </tr>
                ...
                {processedRows.map((task: ProcessedTask) => (
                  <tr key={task.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-normal text-left text-sm font-medium text-[#172B4D]">
                      ...
                      <span className="block text-xs text-[#42526E]">
                        {task.description}
                      </span>
                    </td>
                    {dayHeaders.map((day: string) => (
                      <td
                        key={`${task.id}-${day}`}
                        className="px-6 py-4 whitespace-nowrap text-right text-sm text-[#42526E]"
                      >
                        {task.logs[day] || "—"}
                      </td>
                    ))}
              ...
              <tfoot className="sticky bottom-0 bg-gray-50 border-t-2 border-[#0052CC]">
                <tr>
                  <th
                    scope="row"
                    className="px-6 py-4 text-left text-sm font-extrabold text-[#172B4D]"
                  >
                    GRAND TOTAL:
                  </th>
                  {dayHeaders.map((day: string) => (
                    <th
                      key={`total-${day}`}
                      scope="col"
                      className="px-6 py-4 text-right text-sm font-bold text-[#172B4D] whitespace-nowrap"
                    >
                      {minutesToTime(dailyTotals[day] as unknown as number)}
                    </th>
                  ))}
                  <th
                    scope="col"
                    className="px-6 py-4 text-right text-sm font-extrabold text-[#0052CC] bg-soft-gray/50 whitespace-nowrap"
                  >
